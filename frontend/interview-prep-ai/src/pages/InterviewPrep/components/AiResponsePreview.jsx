...
const AiResponsePreview = ({ content }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-[14px] prase-slate dark:prose-invert max-w-none">
        <ReactMarkdown
          ...
          components={{
            code({ node, inline, className, children, ...props }) {
              ...
              // If it's inline code, render as a simple code tag
              if (inline) {
                return (
              ...
              // For block code, return the CodeBlock component directly (not nested in p)
            ...
            ol({ children }) {
              return <ol className="list-desimal pl-6 space-y-2 my-4">{children}</ol>;
            },
            li({ children }) {
            ...
            th({ children }) {
              return (
                <th className="px-3 py-2 text-left text-xs text-gray-500 uppercase tracking-wider">{children}</th>
              )
            },
            td({ children }) {
              return (
...
function CodeBlock({ code, language }) {
  ...
  return <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
    <div className="flex items-center justify-between bg-gray-100 px-4 py-2 border-gray-200">
    ...
    <SyntaxHighlighter
      language={language}
      style={oneLight}
      customStyle={{ fontSize: 12.5, margin: 0, padding: "1rem", background: "transparant" }}
    >
      {code}
    </SyntaxHighlighter>
  </div>
